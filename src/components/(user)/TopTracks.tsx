...
export default function TopTracks({ userId }: { userId: string }) {
    ...
    const { data: tracks, error, isLoading } = useSWR<Track[]>(
        userId ? `/api/user/${userId}/top-tracks?period=${period}` : null,
        fetcher,
        {
    ...
    const renderTrackCard = (track: Track) => (
        <a 
            href={`/track/${encodeURIComponent(track.trackId)}`} 
            key={track.trackId} 
        ...
        >
            <div className="bg-[#2a211c] rounded-lg p-3 transform transition-transform group-hover:-translate-y-1 shadow-md hover:shadow-lg">
                ...
                <h4 className="font-bold text-[#e6d2c0] line-clamp-1" title={track.name}>
                    {track.name}
                </h4>
                <p className="text-sm text-[#a18072] mt-1 line-clamp-1">
                    {track.artistName}
    ...
    if (isLoading || !tracks) return <FetchLoader />
        ...
        <div className="rounded-lg bg-[#1e1814] border border-[#3d2e23] p-3 lg:p-6 shadow-lg">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-2 lg:mb-4 gap-2 lg:gap-4">
                <div className="flex items-center gap-2">
                    <Music className="size-5 lg:size-6 text-[#c38e70]" aria-hidden="true" />
                    <h2 className="text-lg lg:text-xl font-bold text-[#e6d2c0]">Top Tracks</h2>
                </div>
                <div className="flex items-center gap-2">
