...
export default function ArtistTopTracks({ userId, artistId }) {
    ...
    useEffect(() => {
        const fetchTracks = async () => {
            try {
                const res = await fetch(`/api/getArtistTopTracks/?artistId=${artistId}&userId=${userId}`)
                if (res.ok) {
                            ...
                            <div className="flex gap-4">
                                {tracks.map((track) => (
                                    <a key={track.trackId} href={`/track/${track.trackId}`} className="w-32 lg:w-36 group">
                                        <div className="aspect-square mb-4"><img src={track.trackAlbumImageUrl || "/placeholder.svg"} alt={track.trackName} className="w-full h-full object-cover rounded-md" /></div>
